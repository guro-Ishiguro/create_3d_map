...
# 左右の画像を読み込み
img_id_1 = 1001
left_image_1 = cv2.imread(
    os.path.join(DRONE_IMAGE_DIR, f"left_{img_id_1}.png"), cv2.IMREAD_GRAYSCALE
)
right_image_1 = cv2.imread(
    os.path.join(DRONE_IMAGE_DIR, f"right_{img_id_1}.png"), cv2.IMREAD_GRAYSCALE
)

img_id_2 = 1015
left_image_2 = cv2.imread(
    os.path.join(DRONE_IMAGE_DIR, f"left_{img_id_2}.png"), cv2.IMREAD_GRAYSCALE
)
right_image_2 = cv2.imread(
    os.path.join(DRONE_IMAGE_DIR, f"right_{img_id_2}.png"), cv2.IMREAD_GRAYSCALE
)

img_id_3 = 1024
left_image_3 = cv2.imread(
    os.path.join(DRONE_IMAGE_DIR, f"left_{img_id_3}.png"), cv2.IMREAD_GRAYSCALE
)
right_image_3 = cv2.imread(
    os.path.join(DRONE_IMAGE_DIR, f"right_{img_id_3}.png"), cv2.IMREAD_GRAYSCALE
)
...
# 視差画像を生成
disparity_1 = create_disparity_image(
    left_image_1, right_image_1, window_size=5, min_disp=0, num_disp=80
)
disparity_2 = create_disparity_image(
    left_image_2, right_image_2, window_size=5, min_disp=0, num_disp=80
)
disparity_3 = create_disparity_image(
    left_image_3, right_image_3, window_size=5, min_disp=0, num_disp=80
)
...
world_coords_1 = convert_right_to_left_hand_coordinates(
    depth_to_world(depth_1, K, R, T_1, pixel_size)
)
world_coords_1 = world_coords_1[depth_1.reshape(-1) > 0]

world_coords_2 = convert_right_to_left_hand_coordinates(
    depth_to_world(depth_2, K, R, T_2, pixel_size)
)
world_coords_2 = world_coords_2[depth_2.reshape(-1) > 0]

world_coords_3 = convert_right_to_left_hand_coordinates(
    depth_to_world(depth_3, K, R, T_3, pixel_size)
)
